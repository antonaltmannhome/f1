...
MakeFuelTyreAdjustment = function(multiRaceStatusDT, dataForRace, tyreCoefDT) {

		multiRaceStatusDT$fuelCoef = dataForRace$fuelCoef
		multiRaceStatusDT = tyreCoefDT[multiRaceStatusDT, on = 'tyre']
		multiRaceStatusDT$fuelTyreAdjustment =
			with(multiRaceStatusDT, fuelCoef * fuel + mod30TyreInt + mod30TyreSlope * tyreLap)

		return(multiRaceStatusDT)
}
...
SimulateOvertaking = function(multiRaceStatusDT, numDriver,
																circuitOvertakingCoef, overtakingOverlapCoef,
																numberOfSimulations) {

	### right, now we have to simulate overtaking
	multiRaceStatusDT$predEndTimeElapsed = with(multiRaceStatusDT, startTimeElapsed + predSec)

	### we'll put a nice wrapper around it to make it less ugly at some point.
	## NB multiRaceStatusDF has to be in this order to run
	multiRaceStatusDT = multiRaceStatusDT[order(multiRaceStatusDT$simi, multiRaceStatusDT$startRank),]
	simMatrix = RcppGetOvertakingSummary(multiRaceStatusDT$predEndTimeElapsed,
												numberOfSimulations,
												numDriver,
												circuitOvertakingCoef, overtakingOverlapCoef)
	overtakeSummary = data.table::data.table(driver = multiRaceStatusDT$driver,
																					endRank = simMatrix[,1] + 1L,
																					didOvertake = simMatrix[,2],
																					gotOvertake = simMatrix[,3],
																					simi = rep(1:numberOfSimulations, rep(numDriver, numberOfSimulations)))

	multiRaceStatusDT = overtakeSummary[multiRaceStatusDT, on = c('simi', 'driver')]
...
SimulateLapTime = function(multiRaceStatusDF, secStandardError, startOfLap, currentLap) {
	...
	multiRaceStatusDT = multiRaceStatusDT[order(multiRaceStatusDT$simi, multiRaceStatusDT$endRank),]

	multiRaceStatusDT$endTimeElapsed = with(multiRaceStatusDT,
	...
	# rankingCheckDT = multiRaceStatusDT[,.(checkEndRank = as.integer(data.table::frank(endTimeElapsed))), simi]
	rankingCheckDT = multiRaceStatusDT[,.(checkEndRank = as.integer(rank(endTimeElapsed))), simi]
	multiRaceStatusDT$checkEndRank = rankingCheckDT$checkEndRank
	rankingCheck = with(multiRaceStatusDT, identical(endRank, checkEndRank))
	if (!rankingCheck) {
