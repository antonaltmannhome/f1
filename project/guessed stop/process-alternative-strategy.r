...
ProcessAlternativeStrategy = function(currentDriverStintDF, currentDriverGuessedStopDF = NULL) {
	...
	for (si in 1:nrow(currentDriverStintDF)) {
		...
		if (!currentDriverStintDF$hasGuessedPitStop[si]) {
			...
			if (with(currentDriverStintDF[si,], yesPit & !interrupt)) {
...
stintDF$yesPit = with(stintDF, !stintFinishRace & !isRed)
	...
	if (raceDF$isValidRace30[ri]) {
		...
		for (di in 1:length(currentRaceDriver)) {
			currentDriverStintDF = miniStintDF %>%
									...
									select(driver, stint, startLap, endLap, newTyre, yesPit, interrupt, hasGuessedPitStop) %>%
			...
			myPitStopList[[di]] = ProcessAlternativeStrategy(currentDriverStintDF, currentDriverGuessedStopDF)
