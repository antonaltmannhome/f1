...
DriverRaceCompareSituation = function(myRace, myDriv1, myDriv2, combine = FALSE, toFile = FALSE) {
  ...
  if (!combine) {
    if (toFile) {
      fileOut = MakeRaceFile(myRace, paste0('driver-head-to-head-simulation-position-', myDriv1, '-', myDriv2, '.png'))
      aafunct::NiceGgSave(myPlot = positionPlot, myFile = fileOut)
      fileOut = MakeRaceFile(myRace, paste0('driver-head-to-head-simulation-probability-', myDriv1, '-', myDriv2, '.png'))
      aafunct::NiceGgSave(myPlot = probabilityPlot, myFile = fileOut)
    }
  }
  if (combine) {
