...
MakeIsPredValid = function(smoothParam, raceDriverDT, myRace) {
	...
	if (smoothParam$qrToPredict == 'q') {
		raceDriverDT$isPredValid =
			with(raceDriverDT, hasOtherData & haveRunQualModel & modQualPredNValid > 0 & aggregatedRr <= myrr)
	}
	if (smoothParam$qrToPredict == 'r') {
		raceDriverDT$isPredValid =
			with(raceDriverDT, hasOtherData & haveRunRaceModel & racePredNValid > 0 & aggregatedRr <= myrr)
	}
	if (smoothParam$qrToPredict == 'rfinpos') {
		raceDriverDT$isPredValid =
...
GetSmooth = function(qrToFit, qrToPredict, fwbw, useStretch,
	...
	## myRace is last race you want to include when optimising parameters - i think it's rare you'd want to use this?
	...
	f1smoothing:::DisplayChoice(smoothParam, modelChoice, customSmoothInfo, myRace)
	...
	OOSRddf = f1smoothing:::MakeOOSRddf(smoothParam, rddf, myRace)
	...
	raceDriverDT = f1smoothing:::MakeIsPredValid(smoothParam, raceDriverDT, myRace)
	...
	maxTheta = f1smoothing:::RetrieveMaxTheta(myRace, smoothParam, whatToSmooth)
	...
	if (optParam) {
		...
		maxTheta = f1smoothing:::OptimiseParam(smoothParam, DTList, myRace)
	}
	...
	sqDiff = f1smoothing:::LikFunct(maxTheta, smoothParam, DTList, myRace)
