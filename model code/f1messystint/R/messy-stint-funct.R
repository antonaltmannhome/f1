...
DetectStintIssue = function(stintDF, lbl, isPreDelta) {
	...
	stintDF$isPitStop = with(stintDF, !stintFinishRace & !isRed)
	...
	stintDF$isDryToDryStop=with(stintDF, isPitStop &
	...
	if (!isPreDelta) {
		...
		stintDF$slowInlap = with(stintDF, isDryToDryStop & inlapDelta > 3)
	}

	stintDF = stintDF %>%
				mutate(slowStop = case_when(!couldChangeTyresAndServePenalty ~
												isDryToDryStop & !isSafetyCar & pitStopTime > medianPitStopTime + 3,
											couldChangeTyresAndServePenalty ~
												isDryToDryStop & !isSafetyCar &
	...
	stintDF$shortStint = with(stintDF, !isSafetyCar & isDryToDryStop & endLap - startLap < 10)

	### or with a suspiciously low number of laps left til the end of the race
	stintDF$lateStop = with(stintDF, isDryToDryStop & scheduledLastLap - endLap < 8)

	stintDF$lap1Stop = with(stintDF, isDryToDryStop & endLap <= 2)
...
BolsterStintWithModelInfo = function(stintDF) {
