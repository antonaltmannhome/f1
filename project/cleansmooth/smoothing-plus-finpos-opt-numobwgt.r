...
OptimiseParam = function(smoothParam, DTList, myRace) {
	...
	theta = InitialiseAndProcessCoef(theta = NULL, smoothParam)$theta
...
GetSmooth=function(qrToFit, qrToPredict, fwbw, useStretch,
	...
	rddf = MakePredNValidBin(rddf, smoothParam)
	...
	dum = CalculateSmoothRaceDriverPlusEFinPos(mycoef,	smoothParam, DTList)
	raceDriverDT = dum$raceDriverDT
	OOSRaceDriverDT = dum$OOSRaceDriverDT
	...
	return(list(smoothDF = smoothDF, OOSRaceDriverDT = OOSRaceDriverDT, optcoef = maxTheta, mycoef =mycoef, sqDiff = sqDiff))
}
