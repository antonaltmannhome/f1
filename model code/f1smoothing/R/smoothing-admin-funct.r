...
RetrieveMaxTheta = function(smoothParam, whatToSmooth) {

	smoothModelFile=MakeRaceFile(raceDF$race[nrace], 'smooth-model.csv')
	...
	if (isStandardModel) {
		dum = f1smoothing:::MakeSmoothModelChoiceCombo(smoothParam, whatToSmooth, isStandardModel)
...
DisplayChoice = function(smoothParam, modelChoice, customSmoothInfo) {
...
CheckValidSmoothInfo = function(smoothParam, whatToSmooth) {
	...
	if (!smoothModelChoiceCombo$fwbw %in% c('bw', 'fwbw', 'flat')) {
		stop('fwbw has to be fwbw, bw or flat\n')
	}
	if (!smoothModelChoiceCombo$useStretch %in% c(FALSE, TRUE)) {
		stop('useStretch has to be FALSE or TRUE\n')
	}
}
...
GenerateCombo = function(filterList) {
	...
	allCombo = expand.grid(combolist, stringsAsFactors = FALSE)
...
RunModel=function(myRace, filterList = list(qrToPredict = c('q', 'r')), reset=F) {
	...
	if (length(modelToRun) > 0) {
		...
		for (j in modelToRun) {
			...
			mycoef = f1smoothing:::InitialiseAndProcessCoef(theta = smoothOutput$optcoef,
